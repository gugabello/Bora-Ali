...
st.title("📊 Gráficos Interativos por Cidade")
st.markdown("Análises visuais para **São Paulo, Rio de Janeiro, Recife e Brasília**.")
...
if df_integrado is None or df_integrado.empty:
    ...
    df_long = df_integrado.melt(
        id_vars=['DATA', 'DESTINO'], # <- MUDANÇA (era UF)
        value_vars=['TARIFA', 'TEMP_MEDIA'],
    ...
    chart1 = alt.Chart(df_long).mark_line(point=True).encode(
        ...
        row=alt.Row('DESTINO:N', title="Cidade"), # <- MUDANÇA (era UF)
        tooltip=['DATA:T', 'DESTINO:N', 'Métrica:N', 'Valor:Q'] # <- MUDANÇA (era UF)
    ...
    chart2 = alt.Chart(df_integrado).mark_circle(size=60).encode(
        ...
        color=alt.Color('DESTINO:N', title="Cidade"), # <- MUDANÇA (era UF)
        tooltip=['DATA:T', 'DESTINO:N', 'TARIFA:Q', 'TEMP_MEDIA:Q'] # <- MUDANÇA (era UF)
    ).interactive()
...
if df_ipca is None or df_ipca.empty:
    ...
    # (Este gráfico não muda, pois já era a média de tudo)
